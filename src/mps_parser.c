...
typedef struct
{
    ...
    size_t num_qc_accums;
    size_t qc_accums_capacity;
    int qc_current_accum;
    bool qc_current_is_obj;
...
static int open_qcmatrix_row(MpsParserState *state, const char *row_name);
static int parse_qcmatrix_section(MpsParserState *state, char **tokens, int n_tokens);
...
qp_problem_t *read_mps_file(const char *filename)
{
    ...
    while (fast_reader_gets(reader, line, sizeof(line)))
    {
        ...
        if (n_tokens == 2 && strcmp(tokens[0], "QCMATRIX") == 0)
        {
            if (current_section == SEC_ROWS && !rows_finalized)
            {
                if (finalize_rows(&state) != 0)
                    state.error_flag = 1;
                rows_finalized = true;
            }
            current_section = SEC_QCMATRIX;
            if (open_qcmatrix_row(&state, tokens[1]) != 0)
                state.error_flag = 1;
            continue;
        }
    ...
    size_t qc_kept = 0;
    for (size_t i = 0; i < state.num_qc_accums; ++i)
    {
        if (state.qc_accums[i].coo.nnz > 0)
            qc_kept++;
    }
    if (qc_kept > 0)
    {
        prob->num_quadratic_constraints = (int)qc_kept;
        prob->quadratic_constraint_row_indices = (int *)safe_calloc(qc_kept, sizeof(int));
        prob->quadratic_constraint_matrices = (CsrComponent **)safe_calloc(qc_kept, sizeof(CsrComponent *));
        prob->quadratic_constraint_matrix_num_nonzeros = (int *)safe_calloc(qc_kept, sizeof(int));
        size_t k = 0;
        for (size_t i = 0; i < state.num_qc_accums; ++i)
        {
            QcMatrixAccum *acc = &state.qc_accums[i];
            if (acc->coo.nnz == 0)
                continue;
            if (state.is_maximize)
            {
                for (size_t j = 0; j < acc->coo.nnz; ++j)
                    acc->coo.values[j] *= -1.0;
            }
            prob->quadratic_constraint_row_indices[k] = acc->row_idx;
            prob->quadratic_constraint_matrix_num_nonzeros[k] = (int)acc->coo.nnz;
            CsrComponent *csr = (CsrComponent *)safe_calloc(1, sizeof(CsrComponent));
            if (coo_to_csr_component(csr, &acc->coo, prob->num_variables) != 0)
            {
                fprintf(stderr, "ERROR: Failed to convert QCMATRIX row %d to CSR.\n", acc->row_idx);
                free(csr);
                qp_problem_free(prob);
                free_parser_state(&state);
                return NULL;
            }
            prob->quadratic_constraint_matrices[k] = csr;
            k++;
        }
    }
...
{
    state->qc_current_accum = -1;
    state->qc_current_is_obj = false;

    if (state->objective_row_name && strcmp(row_name, state->objective_row_name) == 0)
    {
        state->qc_current_is_obj = true;
        return 0;
    }
    ...
    if (row_idx == -1)
    {
        fprintf(stderr, "Warning: QCMATRIX references unknown row '%s'. Skipping.\n", row_name);
        return 0;
    }

    for (size_t i = 0; i < state->num_qc_accums; ++i)
    {
        if (state->qc_accums[i].row_idx == row_idx)
        {
            state->qc_current_accum = (int)i;
            return 0;
        }
    }

    if (state->num_qc_accums >= state->qc_accums_capacity)
    {
        size_t new_cap = state->qc_accums_capacity == 0 ? 16 : state->qc_accums_capacity * 2;
        state->qc_accums = (QcMatrixAccum *)safe_realloc(state->qc_accums, new_cap * sizeof(QcMatrixAccum));
        memset(state->qc_accums + state->qc_accums_capacity,
               0,
               (new_cap - state->qc_accums_capacity) * sizeof(QcMatrixAccum));
        state->qc_accums_capacity = new_cap;
    }
    state->qc_accums[state->num_qc_accums].row_idx = row_idx;
    state->qc_current_accum = (int)state->num_qc_accums;
    state->num_qc_accums++;
    return 0;
}
...
static int parse_qcmatrix_section(MpsParserState *state, char **tokens, int n_tokens)
{
    if (n_tokens < 3)
        return 0;
    if (state->qc_current_accum < 0 && !state->qc_current_is_obj)
    {
        return 0;
    }

    const char *col_name1 = tokens[0];
    const char *col_name2 = tokens[1];
    double value = atof(tokens[2]);
    if (value == 0.0)
        return 0;

    int col_idx1 = namemap_get(&state->col_map, col_name1);
    int col_idx2 = namemap_get(&state->col_map, col_name2);
    if (col_idx1 == -1 || col_idx2 == -1)
    {
        fprintf(stderr,
                "Warning: QCMATRIX references unknown variable '%s' or '%s'. "
                "Skipping entry.\n",
                col_name1,
                col_name2);
        return 0;
    ...
    CooMatrix *coo = state->qc_current_is_obj ? &state->coo_matrix_q : &state->qc_accums[state->qc_current_accum].coo;
    return add_coo_entry(coo, col_idx1, col_idx2, value);
}
...
static int coo_to_csr_component(CsrComponent *csr, CooMatrix *coo, size_t num_rows)
{
...
static void free_parser_state(MpsParserState *state)
{
    ...
    if (state->qc_accums)
    {
        for (size_t i = 0; i < state->num_qc_accums; ++i)
        {
            free(state->qc_accums[i].coo.row_indices);
            free(state->qc_accums[i].coo.col_indices);
            free(state->qc_accums[i].coo.values);
        }
        free(state->qc_accums);
    }
