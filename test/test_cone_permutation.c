...
int main(void)
{
    ...
    const char fixed1[] = {0, 0, 1, 1};
    const cone_spec_t cones[] = {
        {.type = CONE_STANDARD_SOC, .start_idx = 1, .v_dim = 1, .alpha = 0.0, .is_fixed = fixed0},
        {.type = CONE_ROTATED_SOC, .start_idx = 6, .v_dim = 2, .alpha = 0.0, .is_fixed = fixed1},
    };

    qp_problem_t *problem =
        create_qp_problem(objective, NULL, NULL, NULL, &A, constraint, constraint, NULL, NULL, NULL, 2, cones);
    CHECK(problem != NULL);
    ...
    for (int cone = 0; cone < 2; ++cone)
    {
        ...
        int length = problem->cones.v_dim[cone] + 2;
